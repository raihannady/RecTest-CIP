...
const insert_product: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const productData: ProductData = {
      ...
      stok: parseInt(stock),
      foto: photo,
      suplier_id: parseInt(supplier),
    };

    try {
      const response = await fetch(
        "http://localhost:3000/api/products/produk",
        {
          method: "POST",
          headers: {
    ...
    <section className="bg-white dark:bg-gray-900">
      <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <form action="#" onSubmit={handleSubmit}>
          ...
          <button
            type="submit"
            className="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-primary-600 bg-primary-700 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800"
          >
            Tambah Produk
          </button>
        </form>
